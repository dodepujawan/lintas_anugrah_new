<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered table-striped" id="KwitansiDgnTable">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Invoice</th>
...
$(document).ready(function() {
    ...
    let tableKwitansi = $('#KwitansiDgnTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('pendinginKwitansi.data') }}",
            data: function (d) {
    ...
    $(document).on('click', '.btn-proses-kwt-dgn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('pendinginKwitansi.proses') }}",
                    type: "POST",
                    data: {
                        invoice: invoice
                    },
                    success: function (response) {
                        if (response.status) {
                            ...
                            // PDF
                            let pdfUrl = "{{ route('pendinginKwitansi.pdfKwitansi', ':invoice') }}";
    ...
    $(document).on('click', '.btn-delete-kwt-dgn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('pendinginKwitansi.destroy') }}",
                    type: "POST",
                    data: {
                        kwt: kwt
                    },
